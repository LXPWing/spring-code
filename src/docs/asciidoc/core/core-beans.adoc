...
* <<beans-annotation-com.Li.config,Annotation-based configuration>>: Spring 2.5 introduced
...
example, `file:C:/com.Li.config/services.xml` or `classpath:/com.Li.config/services.xml`. However, be
...
a DataSource by the name of `subsystemB-dataSource`. When composing the com.Li.spring.main application
that uses both these subsystems, the com.Li.spring.main application refers to the DataSource by the
...
Now each component and the com.Li.spring.main application can refer to the dataSource through a name
...
* Implement the more fine-grained control available with annotation-based configuration,
  as described in <<beans-annotation-com.Li.config>>.
...
`org.springframework.beans.factory.com.Li.config` package) to be useful.
====
...
`org.springframework.beans.factory.com.Li.config.Scope` interface, which is described in this
section. For an idea of how to implement your own scopes, see the `Scope`
implementations that are supplied with the Spring Framework itself and the
{api-spring-framework}/beans/factory/com.Li.config/Scope.html[`Scope`] javadoc,
...
See the {api-spring-framework}/beans/factory/com.Li.config/Scope.html#registerDestructionCallback[javadoc]
...
----
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<bean class="org.springframework.beans.factory.com.Li.config.CustomScopeConfigurer">
...
----
	...
	public final class Boot {
		...
		public static void com.Li.spring.main(final String[] args) throws Exception {
			...
			// com.Li.spring.main method exits, hook is called prior to the app shutting down...
		}
	}
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
...
{api-spring-framework}/beans/factory/com.Li.config/BeanPostProcessor.html[`BeanPostProcessor`]
...
The `org.springframework.beans.factory.com.Li.config.BeanPostProcessor` interface consists of
...
class itself or at least the `org.springframework.beans.factory.com.Li.config.BeanPostProcessor`
...
----
	...
	import org.springframework.beans.factory.com.Li.config.BeanPostProcessor;
...
and arbitrary com.Li.config methods.
...
`org.springframework.beans.factory.com.Li.config.BeanFactoryPostProcessor`. The semantics of
...
{api-spring-framework}/beans/factory/com.Li.config/BeanFactoryPostProcessor.html[`BeanFactoryPostProcessor`]
...
modifying the com.Li.spring.main XML definition file or files for the container.
...
[source,xml,indent=0,subs="verbatim,quotes"]
----
	<bean class="org.springframework.beans.factory.com.Li.config.PropertySourcesPlaceholderConfigurer">
...
[[beans-annotation-com.Li.config]]
...
The `<context:annotation-com.Li.config/>` element implicitly registers the following post-processors:
...
[NOTE]
====
`<context:annotation-com.Li.config/>` only looks for annotations on beans in the same
application context in which it is defined. This means that, if you put
`<context:annotation-com.Li.config/>` in a `WebApplicationContext` for a `DispatcherServlet`,
...
----
	public class MovieRecommender {
		...
		@Autowired
		public void prepare(@Qualifier("com.Li.spring.main") MovieCatalog movieCatalog,
				CustomerPreferenceDao customerPreferenceDao) {
			this.movieCatalog = movieCatalog;
			this.customerPreferenceDao = customerPreferenceDao;
		}
...
----
	class MovieRecommender {
		...
		@Autowired
		fun prepare(@Qualifier("com.Li.spring.main") movieCatalog: MovieCatalog,
					customerPreferenceDao: CustomerPreferenceDao) {
			this.movieCatalog = movieCatalog
			this.customerPreferenceDao = customerPreferenceDao
		}
...
----
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<bean class="example.SimpleMovieCatalog">
			<qualifier value="com.Li.spring.main"/> <1>
...
----
<1> The bean with the `com.Li.spring.main` qualifier value is wired with the constructor argument that
...
can define the bean with an `id` of `com.Li.spring.main` instead of the nested qualifier element, leading
...
semantically express a reference to a unique bean `id`. Good qualifier values are `com.Li.spring.main`
...
(<<beans-annotation-com.Li.config>>) demonstrates how to provide a lot of the configuration
...
`<context:annotation-com.Li.config>`. There is usually no need to include the
`<context:annotation-com.Li.config>` element when using `<context:component-scan>`.
...
`CommonAnnotationBeanPostProcessor` by including the `annotation-com.Li.config` attribute
...
----
	...
	public class SimpleMovieLister {
		...
		@Inject
		public void setMovieFinder(@Named("com.Li.spring.main") MovieFinder movieFinder) {
			this.movieFinder = movieFinder;
		}
...
----
	...
	class SimpleMovieLister {
		...
		@Inject
		fun setMovieFinder(@Named("com.Li.spring.main") movieFinder: MovieFinder) {
			this.movieFinder = movieFinder
		}
...
.Java
----
	public static void com.Li.spring.main(String[] args) {
...
----
	<web-app>
		...
		<servlet>
			...
			<!-- Again, com.Li.config locations must consist of one or more comma- or space-delimited
...
[[beans-java-further-information-java-com.Li.config]]
...
----
	...
	public class ServiceConfig {
		...
		@Bean
		public TransferService transferService() {
			// navigate 'through' the com.Li.config class to the @Bean method!
...
----
	...
	@Configuration
	@Import({ServiceConfig.class, DefaultRepositoryConfig.class})  // import the concrete com.Li.config!
	public class SystemTestConfig {
...
include it within `system-test-com.Li.config.xml` as a `<bean/>` definition. Because
`<context:annotation-com.Li.config/>` is switched on, the container recognizes the
...
The following example shows part of a sample `system-test-com.Li.config.xml` file:
...
----
	public static void com.Li.spring.main(String[] args) {
		ApplicationContext ctx = new ClassPathXmlApplicationContext("classpath:/com/acme/system-test-com.Li.config.xml");
		TransferService transferService = ctx.getBean(TransferService.class);
		// ...
	}
----
...
----
	fun com.Li.spring.main() {
		val ctx = ClassPathXmlApplicationContext("classpath:/com/acme/system-test-com.Li.config.xml")
		val transferService = ctx.getBean<TransferService>()
		// ...
	}
----
...
NOTE: In `system-test-com.Li.config.xml` file, the `AppConfig` `<bean/>` does not declare an `id`
...
describe in the previous example, we can redefine `system-test-com.Li.config.xml` to take advantage of component-scanning.
Note that, in this case, we need not explicitly declare
`<context:annotation-com.Li.config/>`, because `<context:component-scan/>` enables the same
functionality.

The following example shows the modified `system-test-com.Li.config.xml` file:
...
that defines a bean, a properties file, and the `com.Li.spring.main` class) shows how to use
...
----
	@Configuration
	@ImportResource("classpath:/com/acme/properties-com.Li.config.xml")
	public class AppConfig {
...
----
	<beans profile="development"
		...
		<jdbc:embedded-database id="dataSource">
			<jdbc:script location="classpath:com/bank/com.Li.config/sql/schema.sql"/>
			<jdbc:script location="classpath:com/bank/com.Li.config/sql/test-data.sql"/>
...
is populated with additional default property sources including servlet com.Li.config and servlet
...
----
	<beans>
		<import resource="com/bank/service/${customer}-com.Li.config.xml"/>
	</beans>
----
...
.Java
----
	public static void com.Li.spring.main(final String[] args) {
		MessageSource resources = new ClassPathXmlApplicationContext("beans.xml");
