...
The `<aop:com.Li.config>` and `<aop:advisor>` elements are discussed in <<aop-schema>>. The
...
com.Li.spring.main action happens in the `doConcurrentOperation` around advice. Notice that, for the
...
an `<aop:com.Li.config>` element (you can have more than one `<aop:com.Li.config>` element in an
application context configuration). An `<aop:com.Li.config>` element can contain pointcut,
advisor, and aspect elements (note that these must be declared in that order).

WARNING: The `<aop:com.Li.config>` style of configuration makes heavy use of Spring's
...
use either only the `<aop:com.Li.config>` style or only the `AutoProxyCreator` style and
never mix them.
...
You can declare a named pointcut inside an `<aop:com.Li.config>` element, letting the pointcut
...
Here, `dataAccessOperation` is the `id` of a pointcut defined at the top (`<aop:com.Li.config>`)
...
the aspect higher than the transaction advice (we want a fresh transaction each time we
retry). The `maxRetries` and `order` properties are both configured by Spring. The
com.Li.spring.main action happens in the `doConcurrentOperation` around advice method. We try to
...
of the `<aop:com.Li.config>` element to true, as follows:

[source,xml,indent=0,subs="verbatim,quotes"]
----
	<aop:com.Li.config proxy-target-class="true">
...
[NOTE]
====
Multiple `<aop:com.Li.config/>` sections are collapsed into a single unified auto-proxy creator
at runtime, which applies the _strongest_ proxy settings that any of the
`<aop:com.Li.config/>` sections (typically from different XML bean definition files) specified.
...
`<aop:aspectj-autoproxy/>`, or `<aop:com.Li.config/>` elements forces the use of CGLIB
...
The key thing to understand here is that the client code inside the `com.Li.spring.main(..)` method
...
In addition to declaring aspects in your configuration by using either `<aop:com.Li.config>`
...
at the field or method level (see <<beans-annotation-com.Li.config>> for further details).
...
driver class with a `com.Li.spring.main(..)` method to demonstrate the LTW in action:
...
----
	...
	fun com.Li.spring.main(args: Array<String>) {
