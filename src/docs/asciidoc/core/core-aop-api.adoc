...
The com.Li.spring.main example is the `control flow` pointcut.
...
The com.Li.spring.main advantage of a before advice is that there is no need to invoke the `proceed()`
...
As with auto-proxying in general, the com.Li.spring.main point of using `BeanNameAutoProxyCreator` is
...
[source,xml,indent=0,subs="verbatim,quotes"]
----
	<bean id="poolConfigAdvisor" class="org.springframework.beans.factory.com.Li.config.MethodInvokingFactoryBean">
