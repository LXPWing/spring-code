...
The `<aop:com.Li.config/>` definition ensures that the transactional advice defined by the
...
----
	...
	Exception in thread "com.Li.spring.main" java.lang.UnsupportedOperationException at x.y.service.DefaultFooService.insertFoo(DefaultFooService.java:14)
	<!-- AOP infrastructure stack trace elements removed for clarity -->
	at $Proxy0.insertFoo(Unknown Source)
	at Boot.com.Li.spring.main(Boot.java:11)
----
...
boundary and transaction attribute definitions. The com.Li.spring.main difference between imperative
...
The com.Li.spring.main difference when you run the insert by using this second approach is that you do not
...
The com.Li.spring.main advantage of this DAO style is that it depends on Hibernate API only. No import
...
consider using the Spring `context:annotation-com.Li.config` XML element in your application
...
----
	<beans>
		...
		<!-- post-processors for all standard com.Li.config annotations -->
...
The com.Li.spring.main problem with such a DAO is that it always creates a new `EntityManager` through
...
The com.Li.spring.main advantage of this DAO style is that it depends only on the Java Persistence API.
...
`org.springframework.oxm.Marshaller` interface, the com.Li.spring.main method of which follows:
...
The `Marshaller` interface has one com.Li.spring.main method, which marshals the given object to a
...
The application class uses this bean to store its settings. Besides a com.Li.spring.main method, the
class has two methods: `saveSettings()` saves the settings bean to a file named
`settings.xml`, and `loadSettings()` loads these settings again. The following `com.Li.spring.main()` method
...
----
	...
	public class Application {
		...
		public static void com.Li.spring.main(String[] args) throws IOException {
...
[[oxm-schema-based-com.Li.config]]
