...
.Java
----
	@ContextConfiguration("/test-com.Li.config.xml") // <1>
...
`"file:src/com.Li.spring.main/webapp"` for the path to the root of the web application (that is, the
...
----
	@ContextHierarchy({
		@ContextConfiguration("/parent-com.Li.config.xml"),
		@ContextConfiguration("/child-com.Li.config.xml")
	})
...
----
	...
	@Sql(
		...
		com.Li.config = @SqlConfig(commentPrefix = "`", separator = "@@") // <1>
	)
	void userTest() {
...
----
	@Test
	@Sql("/test-user-data.sql", com.Li.config = SqlConfig(commentPrefix = "`", separator = "@@")) // <1>
	fun userTest() {
		// run code that relies on the test data
	}
----
...
----
	@Test
	@SqlGroup({ // <1>
		@Sql(scripts = "/test-schema.sql", com.Li.config = @SqlConfig(commentPrefix = "`")),
...
----
	@Test
	@SqlGroup( // <1>
		Sql("/test-schema.sql", com.Li.config = SqlConfig(commentPrefix = "`")),
		Sql("/test-user-data.sql"))
	fun userTest() {
		// run code that uses the test schema and test data
	}
...
.Java
----
	@SpringJUnitConfig(locations = "/test-com.Li.config.xml") // <1>
...
.Java
----
	@SpringJUnitWebConfig(locations = "/test-com.Li.config.xml") // <1>
...
----
	...
	@ContextConfiguration({"/app-com.Li.config.xml", "/test-data-access-com.Li.config.xml"})
	@ActiveProfiles("dev")
	@Transactional
...
`TestContextManager` is the com.Li.spring.main entry point into the Spring TestContext Framework and is
...
the `TestContextManager` with which the listener is registered. See <<testcontext-tel-com.Li.config>>.
...
[[testcontext-tel-com.Li.config-registering-tels]]
...
[[testcontext-tel-com.Li.config-automatic-discovery]]
...
[[testcontext-tel-com.Li.config-ordering]]
...
through the <<testcontext-tel-com.Li.config-automatic-discovery, aforementioned>>
...
[[testcontext-tel-com.Li.config-merging]]
...
implementations by using the aforementioned <<testcontext-tel-com.Li.config-automatic-discovery,
...
of `AnnotationAwareOrderComparator`, as described in <<testcontext-tel-com.Li.config-ordering>>.
...
* <<testcontext-ctx-management-mixed-com.Li.config>>
...
is treated as an absolute classpath location (for example, `/org/example/com.Li.config.xml`). A
...
----
	@ExtendWith(SpringExtension.class)
	// ApplicationContext will be loaded from "/app-com.Li.config.xml" and
	// "/test-com.Li.config.xml" in the root of the classpath
	@ContextConfiguration(locations={"/app-com.Li.config.xml", "/test-com.Li.config.xml"}) // <1>
	class MyTest {
		// class body...
	}
----
...
----
	...
	// ApplicationContext will be loaded from "/app-com.Li.config.xml" and
	// "/test-com.Li.config.xml" in the root of the classpath
	@ContextConfiguration("/app-com.Li.config.xml", "/test-com.Li.config.xml") // <1>
	class MyTest {
		// class body...
	}
----
...
----
	@ExtendWith(SpringExtension.class)
	// ApplicationContext will be loaded from
	// "/app-com.Li.config.xml" and "/TestConfig.groovy"
	@ContextConfiguration({ "/app-com.Li.config.xml", "/TestConfig.groovy" })
	class MyTest {
		// class body...
	}
----
...
`ExtendedTest` is loaded from `base-com.Li.config.xml` and `extended-com.Li.config.xml`, in that order.
Beans defined in `extended-com.Li.config.xml` can, therefore, override (that is, replace) those
defined in `base-com.Li.config.xml`. The following example shows how one class can extend
...
----
	@ExtendWith(SpringExtension.class)
	// ApplicationContext will be loaded from "/base-com.Li.config.xml"
	// in the root of the classpath
	@ContextConfiguration("/base-com.Li.config.xml") <1>
	class BaseTest {
		// class body...
	}

	// ApplicationContext will be loaded from "/base-com.Li.config.xml" and
	// "/extended-com.Li.config.xml" in the root of the classpath
	@ContextConfiguration("/extended-com.Li.config.xml") <2>
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<beans profile="dev">
			<jdbc:embedded-database id="dataSource">
				<jdbc:script
					location="classpath:com/bank/com.Li.config/sql/schema.sql"/>
				<jdbc:script
					location="classpath:com/bank/com.Li.config/sql/test-data.sql"/>
...
----
	@ExtendWith(SpringExtension.class)
	// ApplicationContext will be loaded from "classpath:/app-com.Li.config.xml"
	@ContextConfiguration("/app-com.Li.config.xml")
	@ActiveProfiles("dev")
...
`app-com.Li.config.xml` configuration file in the root of the classpath. If you inspect
`app-com.Li.config.xml`, you can see that the `accountRepository` bean has a dependency on a
	...
	public class StandaloneDataConfig {
		...
		public DataSource dataSource() {
			return new EmbeddedDatabaseBuilder()
				...
				.addScript("classpath:com/bank/com.Li.config/sql/schema.sql")
				.addScript("classpath:com/bank/com.Li.config/sql/test-data.sql")
				.build();
		}
	}
----
...
`MockServletContext` is set to `src/com.Li.spring.main/webapp`. This is interpreted as a path relative
...
`src/com.Li.spring.main/webapp` is the default location for the root of your WAR. If you need to
...
----
	...
	// defaults to "file:src/com.Li.spring.main/webapp"
	@WebAppConfiguration
...
base path, the resource path effectively defaults to `file:src/com.Li.spring.main/webapp`. Similarly,
...
----
	...
	// classpath resource
	@ContextConfiguration("/spring/test-servlet-com.Li.config.xml")
	class WacTests {
		//...
	}
----
...
----
	...
	// file system resource
	@ContextConfiguration("file:src/com.Li.spring.main/webapp/WEB-INF/servlet-com.Li.config.xml")
	class WacTests {
		//...
	}
----
...
For example, if `TestClassA` specifies `{"app-com.Li.config.xml", "test-com.Li.config.xml"}` for the
...
`{"app-com.Li.config.xml", "test-com.Li.config.xml"}` for its locations (either explicitly or
...
----
	@ExtendWith(SpringExtension.class)
	@WebAppConfiguration
	@ContextConfiguration("file:src/com.Li.spring.main/webapp/WEB-INF/applicationContext.xml")
...
are loaded: one for `/app-com.Li.config.xml`, one for `/user-com.Li.config.xml`, and one for
`{"/user-com.Li.config.xml", "/order-com.Li.config.xml"}`. As with the previous example, the
application context loaded from `/app-com.Li.config.xml` is set as the parent context for the
contexts loaded from `/user-com.Li.config.xml` and `{"/user-com.Li.config.xml", "/order-com.Li.config.xml"}`.
...
application context for `ExtendedTests` is loaded only from `/test-user-com.Li.config.xml` and
has its parent set to the context loaded from `/app-com.Li.config.xml`. The following listing
...
----
	...
	@ContextHierarchy(
		@ContextConfiguration(
			name = "child",
			locations = "/test-user-com.Li.config.xml",
...
----
	...
	@ContextConfiguration("repository-com.Li.config.xml")
...
`@ContextConfiguration` annotation (that is, `repository-com.Li.config.xml`). The following
...
[[testcontext-tx-mgr-com.Li.config]]
...
----
	...
	@Sql(
		...
		com.Li.config = @SqlConfig(transactionMode = ISOLATED)
	)
	@Sql(
...
declared directly by using the `com.Li.config` attribute of the `@Sql` annotation, `@SqlConfig`
...
You can write a plain unit test for a Spring MVC com.Li.controller by using JUnit or TestNG. To
do so, instantiate the com.Li.controller, inject it with mocked or stubbed dependencies, and
...
Furthermore, other com.Li.controller methods such as `@InitBinder`, `@ModelAttribute`, and
...
You have two com.Li.spring.main options for creating an instance of `MockMvc`. The first is to load
...
Your second option is to manually create a com.Li.controller instance without loading Spring
...
com.Li.controller at a time. You can manually inject the com.Li.controller with mock dependencies, and
it does not involve loading Spring configuration. Such tests are more focused on style
and make it easier to see which com.Li.controller is being tested, whether any specific Spring
...
.Java
----
	mockMvc.perform(get("/app/com.Li.spring.main/hotels/{id}").contextPath("/app").servletPath("/com.Li.spring.main"))
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
	...
	mockMvc.get("/app/com.Li.spring.main/hotels/{id}") {
		contextPath = "/app"
		servletPath = "/com.Li.spring.main"
	}
----
	...
	class MyWebTests {
		...
		void setup() {
			mockMvc = standaloneSetup(new AccountController())
				.defaultRequest(get("/")
				.contextPath("/app").servletPath("/com.Li.spring.main")
...
inspect Spring MVC specific aspects, such as which com.Li.controller method processed the
...
Below is an example test for com.Li.controller methods that return `DeferredResult`, `Callable`,
...
the stand-alone setup, focusing on one com.Li.controller at a time and manually providing the
...
This test has some obvious drawbacks. If we update our com.Li.controller to use the parameter
`message` instead of `text`, our form test continues to pass, even though the HTML form
is out of synch with the com.Li.controller. To resolve this we can combine our two tests, as
follows:
