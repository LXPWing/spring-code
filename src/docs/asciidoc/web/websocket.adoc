...
[subs="verbatim,quotes"]
----
	import org.springframework.web.socket.com.Li.config.annotation.EnableWebSocket;
	import org.springframework.web.socket.com.Li.config.annotation.WebSocketConfigurer;
	import org.springframework.web.socket.com.Li.config.annotation.WebSocketHandlerRegistry;
...
Neither of these mechanism makes it possible to use a single "`front com.Li.controller`"
...
[.small]#<<web-reactive.adoc#webflux-websocket-server-com.Li.config, WebFlux>>#
...
WebSocket Java com.Li.config, as the following example shows:
...
that into Spring's `DefaultHandshakeHandler` through your WebSocket Java com.Li.config.
...
[subs="verbatim,quotes"]
----
	import org.springframework.web.socket.com.Li.config.annotation.EnableWebSocketMessageBroker;
	import org.springframework.web.socket.com.Li.config.annotation.StompEndpointRegistry;
	...
	public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
		...
		@Override
		public void configureMessageBroker(MessageBrokerRegistry com.Li.config) {
			com.Li.config.setApplicationDestinationPrefixes("/app"); // <2>
			com.Li.config.enableSimpleBroker("/topic", "/queue"); // <3>
		}
	}
----
...
[[websocket-stomp-server-com.Li.config]]
=== WebSocket Server
...
The com.Li.spring.main difference between the two preceding diagrams is the use of the "`broker relay`" for passing
...
com.Li.controller can also do the same in response to HTTP requests, so a client can perform an
...
some `/app` destination, and a com.Li.controller could return a value in response to that
...
a broker or a com.Li.controller processes a given message first. If the goal is to be notified
...
<<web.adoc#mvc-ann-com.Li.controller-advice, similar support>> available in Spring MVC.
...
After that, a com.Li.controller can use a dot (`.`) as the separator in `@MessageMapping` methods,
...
you need to ensure that the authentication `ChannelInterceptor` com.Li.config is ordered
...
inbound client messages and may be accessed from a com.Li.controller method, as the following example shows:
...
time it is accessed from the com.Li.controller and stores the instance in the WebSocket
...
There are two com.Li.spring.main approaches to testing applications when you use Spring's STOMP-over-WebSocket
...
The simplest form of server-side tests is to write com.Li.controller unit tests. However,
this is not useful enough, since much of what a com.Li.controller does depends on its
...
use it to send messages to be handled by com.Li.controller methods.
