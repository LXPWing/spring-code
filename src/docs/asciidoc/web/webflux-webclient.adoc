...
[NOTE]
====
With `Flux` or `Mono`, you should never have to block in a Spring MVC or Spring WebFlux com.Li.controller.
Simply return the resulting reactive type from the com.Li.controller method. The same principle apply to
Kotlin Coroutines and Spring WebFlux, just use suspending function or return `Flow` in your
com.Li.controller method .
====
