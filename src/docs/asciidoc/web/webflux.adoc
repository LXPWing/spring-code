...
can then write to the response. The com.Li.spring.main purpose of Reactive Streams is to let the
...
* <<webflux-com.Li.controller>>: Consistent with Spring MVC and based on the same annotations
...
directly from Spring MVC com.Li.controller methods. The greater the latency per call or the
...
<<webflux-com.Li.config, WebFlux infrastructure>> and <<webflux-httphandler, run it>> with a few
lines of code.
...
intentionally minimal, and its com.Li.spring.main and only purpose is to be a minimal abstraction
...
----
	...
	Tomcat.addServlet(rootContext, "com.Li.spring.main", servlet);
	rootContext.addServletMappingDecoded("/", "com.Li.spring.main");
	server.setHost(host);
	server.setPort(port);
	server.start();
----
...
`HttpMessageReader<Part>` instead. For example, in an annotated com.Li.controller, use of
...
`WebHandler`. When using the <<webflux-com.Li.config>>, registering a `WebFilter` is as simple
...
<<webflux-com.Li.config>>, registering a `WebExceptionHandler` is as simple as declaring it as a
...
| Extension of `ResponseStatusExceptionHandler` that can also determine the HTTP status
  ...
  This handler is declared in the <<webflux-com.Li.config>>.
...
<<webflux-com.Li.config-message-codecs>>.
...
object — for example, a com.Li.controller method with `@RequestBody byte[]`,
...
set all codecs, see <<webflux-com.Li.config-message-codecs>>. On the client side, the limit for
...
Spring WebFlux, similarly to Spring MVC, is designed around the front com.Li.controller pattern,
...
| Map a request to a handler. The mapping is based on some criteria, the details of
  ...
  The com.Li.spring.main `HandlerMapping` implementations are `RequestMappingHandlerMapping` for
...
| Help the `DispatcherHandler` to invoke a handler mapped to a request regardless of
  ...
  requires resolving annotations. The com.Li.spring.main purpose of a `HandlerAdapter` is to shield the
...
[[webflux-framework-com.Li.config]]
=== WebFlux Config
[.small]#<<web.adoc#mvc-servlet-com.Li.config, Web MVC>>#
...
However, in most cases, the <<webflux-com.Li.config>> is the best starting point. It declares the
required beans and provides a higher-level configuration callback API to customize it.

NOTE: Spring Boot relies on the WebFlux com.Li.config to configure Spring WebFlux and also provides
...
`HandlerResultHandler` implementations, all of which are declared in the <<webflux-com.Li.config>>:
...
See also <<webflux-ann-com.Li.controller-exceptions>> in the "`Annotated Controller`" section or
...
to your Spring configuration. <<webflux-com.Li.config-view-resolvers, WebFlux Config>> provides a
...
The net effect is the same as if the com.Li.controller had returned a `RedirectView` or
`Rendering.redirectTo("abc").build()`, but now the com.Li.controller itself can
...
views through the <<webflux-com.Li.config-view-resolvers, WebFlux Configuration>>. Default views are
always selected and used if they match the requested media type.




[[webflux-com.Li.controller]]
== Annotated Controllers
[.small]#<<web.adoc#mvc-com.Li.controller, Web MVC>>#
...
[[webflux-ann-com.Li.controller]]
=== `@Controller`
[.small]#<<web.adoc#mvc-ann-com.Li.controller, Web MVC>>#

You can define com.Li.controller beans by using a standard Spring bean definition.
...
itself meta-annotated with `@Controller` and `@ResponseBody`, indicating a com.Li.controller whose
...
because, arguably, most com.Li.controller methods should be mapped to a specific HTTP method versus
...
The following table shows the supported com.Li.controller method arguments.
...
The following table shows the supported com.Li.controller method return values. Note that reactive
...
| A method with a `void`, possibly asynchronous (for example, `Mono<Void>`), return type (or a `null` return
  ...
  if the com.Li.controller has made a positive ETag or `lastModified` timestamp check.
...
Some annotated com.Li.controller method arguments that represent String-based request input (for example,
...
com.Li.controller method, you need to add a URI variable to the path segment where matrix
...
com.Li.controller. The following code snippet shows the usage:
...
to check for such errors in the com.Li.controller method, you can add a `BindingResult` argument
...
another com.Li.controller method uses a `SessionStatus` method argument to clear the storage,
as the following example shows:
...
`WebSession` into the com.Li.controller method.
...
content. The best way to handle a file upload form (for example, from a browser) in a com.Li.controller
...
in the com.Li.controller method by declaring the argument with an async wrapper and then using
...
You can use the <<webflux-com.Li.config-message-codecs>> option of the <<webflux-com.Li.config>> to
...
com.Li.controller method by declaring the argument with an async wrapper and then using error
related operators:
...
com.Li.controller method. Use a composite interface if you need to activate multiple views.
...
<<webflux-ann-com.Li.controller-advice>> for more details.
...
<<webflux-com.Li.config-conversion, WebFlux Java configuration>> to register `Converter` and
...
com.Li.controller-specific `Formatter` instances, as the following example shows:
...
[[webflux-ann-com.Li.controller-exceptions]]
=== Managing Exceptions
[.small]#<<web.adoc#mvc-ann-exceptionhandler, Web MVC>>#

`@Controller` and <<webflux-ann-com.Li.controller-advice, @ControllerAdvice>> classes can have
`@ExceptionHandler` methods to handle exceptions from com.Li.controller methods. The following
...
[[webflux-ann-com.Li.controller-advice]]
=== Controller Advice
[.small]#<<web.adoc#mvc-ann-com.Li.controller-advice, Web MVC>>#
...
against conditional request headers. A com.Li.controller can add an `ETag` and `Cache-Control`
...
for optimal performance. See the section on configuring <<webflux-com.Li.config-static-resources>>.




[[webflux-com.Li.config]]
== WebFlux Config
[.small]#<<web.adoc#mvc-com.Li.config, Web MVC>>#
...
<<webflux-com.Li.config-advanced-java>>.



[[webflux-com.Li.config-enable]]
=== Enabling WebFlux Config
[.small]#<<web.adoc#mvc-com.Li.config-enable, Web MVC>>#

You can use the `@EnableWebFlux` annotation in your Java com.Li.config, as the following example shows:
...
[[webflux-com.Li.config-customize]]
=== WebFlux com.Li.config API
[.small]#<<web.adoc#mvc-com.Li.config-customize, Web MVC>>#
...
[[webflux-com.Li.config-conversion]]
=== Conversion, formatting
[.small]#<<web.adoc#mvc-com.Li.config-conversion, Web MVC>>#
...
[[webflux-com.Li.config-validation]]
=== Validation
[.small]#<<web.adoc#mvc-com.Li.config-validation, Web MVC>>#
...
mark it with `@Primary` in order to avoid conflict with the one declared in the MVC com.Li.config.



[[webflux-com.Li.config-content-negotiation]]
=== Content Type Resolvers
[.small]#<<web.adoc#mvc-com.Li.config-content-negotiation, Web MVC>>#
...
[[webflux-com.Li.config-message-codecs]]
=== HTTP message codecs
[.small]#<<web.adoc#mvc-com.Li.config-message-converters, Web MVC>>#
...
[[webflux-com.Li.config-view-resolvers]]
=== View Resolvers
[.small]#<<web.adoc#mvc-com.Li.config-view-resolvers, Web MVC>>#
...
[[webflux-com.Li.config-static-resources]]
=== Static Resources
[.small]#<<web.adoc#mvc-com.Li.config-static-resources, Web MVC>>#
...
[[webflux-com.Li.config-path-matching]]
=== Path Matching
[.small]#<<web.adoc#mvc-com.Li.config-path-matching, Web MVC>>#

You can customize options related to path matching. For details on the individual options, see the
{api-spring-framework}/web/reactive/com.Li.config/PathMatchConfigurer.html[`PathMatchConfigurer`] javadoc.
...
[.small]#<<web.adoc#mvc-com.Li.config-advanced-java, Web MVC>>#
