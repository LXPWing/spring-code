...
Spring MVC, as many other web frameworks, is designed around the front com.Li.controller
...
(see <<mvc-container-com.Li.config>>):

[source,java,indent=0,subs="verbatim,quotes",role="primary"]
.Java
----
	public class com.Li.MyWebApplicationInitializer implements WebApplicationInitializer {
...
.Kotlin
----
	class com.Li.MyWebApplicationInitializer : WebApplicationInitializer {
...
| Map a request to a handler along with a list of
  ...
  The two com.Li.spring.main `HandlerMapping` implementations are `RequestMappingHandlerMapping`
...
| Help the `DispatcherServlet` to invoke a handler mapped to a request, regardless of
  how the handler is actually invoked. For example, invoking an annotated com.Li.controller
  requires resolving annotations. The com.Li.spring.main purpose of a `HandlerAdapter` is
...
[[mvc-servlet-com.Li.config]]
=== Web MVC Config
[.small]#<<web-reactive.adoc#webflux-framework-com.Li.config, WebFlux>>#
...
https://github.com/spring-projects/spring-framework/blob/master/spring-webmvc/src/com.Li.spring.main/resources/org/springframework/web/servlet/DispatcherServlet.properties[`DispatcherServlet.properties`].

In most cases, the <<mvc-com.Li.config>> is the best starting point. It declares the required
...
----
	public class MyWebAppInitializer extends AbstractDispatcherServletInitializer {
		...
		protected WebApplicationContext createServletApplicationContext() {
			...
			cxt.setConfigLocation("/WEB-INF/spring/dispatcher-com.Li.config.xml");
			return cxt;
		}
	...
	class MyWebAppInitializer : AbstractDispatcherServletInitializer() {
		...
		override fun createServletApplicationContext(): WebApplicationContext {
			return XmlWebApplicationContext().apply {
				setConfigLocation("/WEB-INF/spring/dispatcher-com.Li.config.xml")
			}
		}
...
* The `WebApplicationContext` is searched for and bound in the request as an attribute
  that the com.Li.controller and other elements in the process can use. It is bound by default
...
| Whether to throw a `NoHandlerFoundException` when no handler was found for a request.
  ...
  `@ExceptionHandler` com.Li.controller method) and handled as any others.
...
See <<mvc-com.Li.config-interceptors>> in the section on MVC configuration for examples of how to
...
declare it as an <<mvc-ann-com.Li.controller-advice>> bean or configure it directly on
...
The <<mvc-com.Li.config>> automatically declares built-in resolvers for default Spring MVC
...
configuration. The <<mvc-com.Li.config>> provides a dedicated configuration API for
<<mvc-com.Li.config-view-resolvers>> and for adding logic-less
<<mvc-com.Li.config-view-com.Li.controller, View Controllers>> which are useful for HTML template
rendering without com.Li.controller logic.
...
The net effect is the same as if the com.Li.controller had returned a `RedirectView`, but now
the com.Li.controller itself can operate in terms of logical view names. A logical view
...
Note that, if a com.Li.controller method is annotated with the `@ResponseStatus`, the annotation
...
See <<mvc-com.Li.config-view-resolvers>> under <<mvc-com.Li.config>> for configuration details.
...
* In `web.xml`, add a `"<multipart-com.Li.config>"` section to the servlet declaration.
...
(see <<mvc-container-com.Li.config>>) all filters are automatically registered for all dispatch
...
for each request. Other strategies at the com.Li.controller level, described earlier, can avoid
...
`AbstractAnnotationConfigDispatcherServletInitializer` (see <<mvc-container-com.Li.config>>)
...
[[mvc-com.Li.controller]]
== Annotated Controllers
[.small]#<<web-reactive.adoc#webflux-com.Li.controller, WebFlux>>#
...
The following example shows a com.Li.controller defined by annotations:
...
[[mvc-ann-com.Li.controller]]
=== Declaration
[.small]#<<web-reactive.adoc#webflux-ann-com.Li.controller, WebFlux>>#

You can define com.Li.controller beans by using a standard Spring bean definition in the
...
itself meta-annotated with `@Controller` and `@ResponseBody` to indicate a com.Li.controller whose
...
In some cases, you may need to decorate a com.Li.controller with an AOP proxy at runtime.
One example is if you choose to have `@Transactional` annotations directly on the
com.Li.controller. When this is the case, for controllers specifically, we recommend
using class-based proxying. This is typically the default choice with controllers.
However, if a com.Li.controller must implement an interface that is not a Spring Context
...
arguably, most com.Li.controller methods should be mapped to a specific HTTP method versus
...
See <<mvc-com.Li.config-path-matching>> in the configuration section.
...
com.Li.controller mapped to `/person` is also implicitly mapped to `/person.{asterisk}`.
...
* `useSuffixPatternMatching(false)`, see <<mvc-com.Li.config-path-matching, PathMatchConfigurer>>
* `favorPathExtension(false)`, see <<mvc-com.Li.config-content-negotiation, ContentNegotiationConfigurer>>
...
`mediaTypes` property of <<mvc-com.Li.config-content-negotiation,ContentNegotiationConfigurer>>.
...
See <<mvc-com.Li.config-content-negotiation>>.
...
content types that a com.Li.controller method produces, as the following example shows:
...
com.Li.controller methods should be mapped to a specific HTTP method versus using `@RequestMapping`,
...
supported com.Li.controller method arguments and return values.
...
The next table describes the supported com.Li.controller method arguments. Reactive types are not supported
for any arguments.
...
The next table describes the supported com.Li.controller method return values. Reactive types are
...
| A method with a `void` return type (or `null` return value) is considered to have fully
  ...
  an `@ResponseStatus` annotation. The same is also true if the com.Li.controller has made a positive
...
Some annotated com.Li.controller method arguments that represent `String`-based request input (such as
...
If a URL is expected to contain matrix variables, the request mapping for a com.Li.controller
...
<<mvc-com.Li.config-path-matching>>. In the MVC XML namespace, you can set
...
query parameters or form data) to a method argument in a com.Li.controller.
...
for such errors in the com.Li.controller method, you can add a `BindingResult` argument immediately next
...
cases, you can inject the `Model` into the com.Li.controller and access it directly or,
...
specific com.Li.controller. This typically lists the names of model attributes or types of
...
until another com.Li.controller method uses a `SessionStatus` method argument to clear the
...
(that is, outside the com.Li.controller -- for example, by a filter) and may or may not be present,
...
`javax.servlet.http.HttpSession` into the com.Li.controller method.

For temporary storage of model attributes in the session as part of a com.Li.controller
...
`Model` should never be used if a com.Li.controller method redirects. Instead, the com.Li.controller
...
Spring MVC has two com.Li.spring.main abstractions in support of flash attributes. `FlashMap` is used
...
`Model` of the com.Li.controller that serves the target URL.
...
within the com.Li.controller through an `Errors` or `BindingResult` argument,
as the following example shows:
...
You can use the <<mvc-com.Li.config-message-converters>> option of the <<mvc-com.Li.config>> to
...
all com.Li.controller methods. This is the effect of `@RestController`, which is nothing more
...
`@ResponseBody` or `ResponseEntity` com.Li.controller methods, you can use Jackson's
...
com.Li.controller method. If you need to activate multiple views, you can use a composite interface.
...
A com.Li.controller can have any number of `@ModelAttribute` methods. All such methods are
invoked before `@RequestMapping` methods in the same com.Li.controller. A `@ModelAttribute`
method can also be shared across controllers through `@ControllerAdvice`. See the section on
<<mvc-ann-com.Li.controller-advice>> for more details.
...
headers, cookies, and others) to the target type of com.Li.controller method arguments.
* Format model object values as `String` values when rendering HTML forms.

`@InitBinder` methods can register com.Li.controller-specific `java.bean.PropertyEditor` or
Spring `Converter` and `Formatter` components. In addition, you can use the
<<mvc-com.Li.config-conversion,MVC com.Li.config>> to register `Converter` and `Formatter`
...
com.Li.controller-specific `Formatter` implementations, as the following example shows:
...
=== Exceptions
[.small]#<<web-reactive.adoc#webflux-ann-com.Li.controller-exceptions, WebFlux>>#

`@Controller` and <<mvc-ann-com.Li.controller-advice, @ControllerAdvice>> classes can have
`@ExceptionHandler` methods to handle exceptions from com.Li.controller methods, as the following example shows:
...
com.Li.controller or `@ControllerAdvice` class. This means a cause match on a higher-priority
...
| `HandlerMethod`
| For access to the com.Li.controller method that raised the exception.
...
| A method with a `void` return type (or `null` return value) is considered to have fully
  handled the response if it also has a `ServletResponse` an `OutputStream` argument, or
  a `@ResponseStatus` annotation. The same is also true if the com.Li.controller has made a positive
  `ETag` or `lastModified` timestamp check (see <<mvc-caching-etag-lastmodified>> for details).
...
[[mvc-ann-com.Li.controller-advice]]
=== Controller Advice
[.small]#<<web-reactive.adoc#webflux-ann-com.Li.controller-advice, WebFlux>>#
...
You can create URIs relative to a Servlet (for example, `/com.Li.spring.main/{asterisk}`),
as the following example shows:
...
Spring MVC provides a mechanism to prepare links to com.Li.controller methods. For example,
the following MVC com.Li.controller allows for link creation:
...
akin to mock testing through proxies to avoid referring to the com.Li.controller method by name, as the following example shows
...
Given the preceding com.Li.controller, you can prepare a link from a JSP, as follows:
...
return values in com.Li.controller methods and provide basic support for a single asynchronous
return value.
...
in the Servlet container, com.Li.controller methods can wrap any supported com.Li.controller method
...
The com.Li.controller can produce the return value asynchronously, from a different thread -- for
...
A com.Li.controller can wrap any supported return value with `java.util.concurrent.Callable`,
as the following example shows:
...
* A `ServletRequest` can be put in asynchronous mode by calling `request.startAsync()`.
  The com.Li.spring.main effect of doing so is that the Servlet (as well as any filters) can exit, but
...
* The com.Li.controller returns a `DeferredResult` and saves it in some in-memory
...
* The com.Li.controller returns a `Callable`.
...
com.Li.controller method returned the given value or as if it produced the given exception.
The exception then goes through the regular exception handling mechanism (for example, invoking
`@ExceptionHandler` methods).

When you use `Callable`, similar processing logic occurs, the com.Li.spring.main difference being that
...
asynchronous support is built around that mechanism. When a com.Li.controller returns a `DeferredResult`,
...
com.Li.controller is mapped again but, rather than invoking it, the `DeferredResult` value is used
(as if the com.Li.controller returned it) to resume processing.
...
asynchronous and <<mvc-ann-async-reactive-types>> as return values in com.Li.controller methods.
...
types in com.Li.controller method arguments (for example, `@RequestBody`, `@RequestPart`, and others),
...
stream from a com.Li.controller, return `SseEmitter`, as the following example shows:
...
While SSE is the com.Li.spring.main option for streaming into browsers, note that Internet Explorer
...
Spring MVC supports use of reactive client libraries in a com.Li.controller (also read
...
reactive types from the com.Li.controller method.
...
com.Li.controller methods. We highly recommended configuring this property if you
stream with reactive types or have com.Li.controller methods that return `Callable`, since
...
against conditional request headers. A com.Li.controller can add an `ETag` header and `Cache-Control`
...
You can also make the check against conditional request headers in the com.Li.controller,
as the following example shows:
...
for optimal performance. See the section on configuring <<mvc-com.Li.config-static-resources>>.
...
[[mvc-com.Li.config]]
== MVC Config
[.small]#<<web-reactive.adoc#webflux-com.Li.config, WebFlux>>#
...
see <<mvc-com.Li.config-advanced-java>> and <<mvc-com.Li.config-advanced-xml>>.
...
and <<mvc-servlet-com.Li.config>>.



[[mvc-com.Li.config-enable]]
=== Enable MVC Configuration
[.small]#<<web-reactive.adoc#webflux-com.Li.config-enable, WebFlux>>#
...
[[mvc-com.Li.config-customize]]
=== MVC Config API
[.small]#<<web-reactive.adoc#webflux-com.Li.config-customize, WebFlux>>#
...
[[mvc-com.Li.config-conversion]]
=== Type Conversion
[.small]#<<web-reactive.adoc#webflux-com.Li.config-conversion, WebFlux>>#

By default, formatters for various number and date types are installed, along with support
for customization via `@NumberFormat` and `@DateTimeFormat` on fields.

To register custom formatters and converters in Java com.Li.config, use the following:
...
To do the same in XML com.Li.config, use the following:
...
[[mvc-com.Li.config-validation]]
=== Validation
[.small]#<<web-reactive.adoc#webflux-com.Li.config-validation, WebFlux>>#
...
`Validated` on com.Li.controller method arguments.
...
[[mvc-com.Li.config-interceptors]]
=== Interceptors
...
[[mvc-com.Li.config-content-negotiation]]
=== Content Types
[.small]#<<web-reactive.adoc#webflux-com.Li.config-content-negotiation, WebFlux>>#
...
[[mvc-com.Li.config-message-converters]]
=== Message Converters
[.small]#<<web-reactive.adoc#webflux-com.Li.config-message-codecs, WebFlux>>#

You can customize `HttpMessageConverter` in Java configuration by overriding
{api-spring-framework}/web/servlet/com.Li.config/annotation/WebMvcConfigurer.html#configureMessageConverters-java.util.List-[`configureMessageConverters()`]
(to replace the default converters created by Spring MVC) or by overriding
{api-spring-framework}/web/servlet/com.Li.config/annotation/WebMvcConfigurer.html#extendMessageConverters-java.util.List-[`extendMessageConverters()`]
...
[[mvc-com.Li.config-view-com.Li.controller]]
=== View Controllers

This is a shortcut for defining a `ParameterizableViewController` that immediately
forwards to a view when invoked. You can use it in static cases when there is no Java com.Li.controller
...
using the `<mvc:view-com.Li.controller>` element:

[source,xml,indent=0,subs="verbatim,quotes"]
----
	<mvc:view-com.Li.controller path="/" view-name="home"/>
----

If an `@RequestMapping` method is mapped to a URL for any HTTP method then a view
com.Li.controller cannot be used to handle the same URL. This is because a match by URL to an
annotated com.Li.controller is considered a strong enough indication of endpoint ownership so
...
splitting URL handling across an annotated com.Li.controller and a view com.Li.controller.



[[mvc-com.Li.config-view-resolvers]]
=== View Resolvers
[.small]#<<web-reactive.adoc#webflux-com.Li.config-view-resolvers, WebFlux>>#
...
[[mvc-com.Li.config-static-resources]]
=== Static Resources
[.small]#<<web-reactive.adoc#webflux-com.Li.config-static-resources, WebFlux>>#
...
[[mvc-com.Li.config-path-matching]]
=== Path Matching
[.small]#<<web-reactive.adoc#webflux-com.Li.config-path-matching, WebFlux>>#

You can customize options related to path matching and treatment of the URL.
For details on the individual options, see the
{api-spring-framework}/web/servlet/com.Li.config/annotation/PathMatchConfigurer.html[`PathMatchConfigurer`] javadoc.
...
[[mvc-com.Li.config-advanced-java]]
=== Advanced Java Config
[.small]#<<web-reactive.adoc#webflux-com.Li.config-advanced-java, WebFlux>>#
...
[[mvc-com.Li.config-advanced-xml]]
=== Advanced XML Config
