...
----
	...
	public class Server {
		...
		public static void com.Li.spring.main(String[] args) throws Exception {
...
The com.Li.spring.main reason why auto-detection of implemented interfaces does not occur for remote
...
Concrete implementations for the com.Li.spring.main media (MIME) types are provided in the framework
...
<<web.adoc#mvc-com.Li.config-message-converters, Configuring Message Converters>>).
...
Assume that we have a web com.Li.controller that needs to use a local EJB. We follow best
...
One of the com.Li.spring.main reasons to use the Business Methods Interface pattern is to ensure that
...
com.Li.controller to the EJB implementation is to expose a setter method of type `MyComponent`
on the com.Li.controller. This saves the reference as an instance variable in the
com.Li.controller. The following example shows how to do so:
...
We can subsequently use this instance variable in any business method in the com.Li.controller.
Now, assuming we obtain our com.Li.controller object out of a Spring container, we can
...
`myComponent` property of the com.Li.controller with the following configuration entry:
...
The `myController` bean definition sets the `myComponent` property of the com.Li.controller
...
The `org.springframework.jms.com.Li.config` package provides the parser implementation for the
`jms` namespace as well as the java com.Li.config support to configure listener containers and
...
----
	...
	<bean id="jmsListenerContainerFactory"
			class="org.springframework.jms.com.Li.config.DefaultJmsListenerContainerFactory">
		<property name="connectionFactory" ref="connectionFactory"/>
...
The com.Li.spring.main elements you can inject in JMS listener endpoints are as follows:
...
case (through setting the 'proxy-target-class' flag on `<aop:com.Li.config/>`,
...
the com.Li.spring.main open-source JMX implementation, MX4J, and the one provided with the JDK
...
* <<cci-com.Li.config-connector>>
* <<cci-com.Li.config-connectionfactory>>
* <<cci-com.Li.config-cci-connections>>
* <<cci-com.Li.config-single-connection>>
...
The com.Li.spring.main idea is that, when a task is submitted, the executor first tries to use a
...
[[cache-annotations-com.Li.config]]
...
describes the com.Li.spring.main differences between the Spring annotations and the JSR-107
counterpart:
...
The `aop:com.Li.config` definition applies the cache advice to the appropriate points in the
...
----
	...
	<bean id="ehcache"
			class="org.springframework.cache.ehcache.EhCacheManagerFactoryBean" p:com.Li.config-location="ehcache.xml"/>
----
